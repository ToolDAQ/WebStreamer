...
void Channel::Send(const std::string &message, std::string filter){
  ...
  mtx.lock_shared();
  for(auto it = filtered_clients.begin(); it != filtered_clients.end(); it++){
    if(it->first.find(filter) != std::string::npos && it->first != "*"){
      for(auto client = it->second.begin(); client != it->second.end(); client++){
  ...
  for(auto client = filtered_clients["*"].begin(); client != filtered_clients["*"].end(); client++){
